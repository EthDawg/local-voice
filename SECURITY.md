...
This is an early community project. Security fixes target the current `main` branch; older release binaries are not a maintained security branch. Reports are handled on a best-effort basis with no guaranteed response time. The maintainer will coordinate a fix and disclosure with the reporter.

Local processing does not mean encrypted storage. See the README for data locations, permissions, and retention. Dependencies and downloaded models retain their own security and licensing terms.
