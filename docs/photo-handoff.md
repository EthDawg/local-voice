...
- Both signed native builds passed. The Developer ID Mac Preview is installed with existing data preserved; its installed signature/profile/configuration passed preflight, and its normal **From iPhone** view successfully checked the live private iCloud account. The iOS archive and Release Testing export passed signature/configuration checks, including the exported app's signing-certificate membership in its profile and authorization of the intended phone. Installation on that phone succeeded; normal launch and the real paired photo transfer remain unverified.
- Later [CI at `2b571e4`](https://github.com/EthDawg/local-voice/actions/runs/34746098779) passed Mac and iPad. The iPhone job passed 18 unit tests and six UI journeys, but the local-photo journey could not focus its name field before typing. The test now waits for the native keyboard and verifies the entered name; that complete journey passed three consecutive runs on local iOS 26.5. CI used iOS 26.4, so a new CI result remains separate evidence. No application behavior changed.
