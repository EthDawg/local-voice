...
| Mac desktop, non-App-Store build | More → Use as animated desktop. Verify a native rendered still, then start an independent click-through layer on that display. Pause/Resume/Stop appear in the scene window. | The applied PNG, recovery records and the saved scene. Quit or Stop removes the layer; the native still remains. |
