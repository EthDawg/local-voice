...
| Native scene-file round trip | Native Export/Import panels produced an independent scene ID, retaining all three assets: background, portrait and rendered card. The renamed copy and original both received matching cloud revision receipts. | Mobile file exchange, interrupted/malformed imports and recovery after subsequent cross-device edits. |
...
Remaining acceptance includes airplane-mode creation/restart; edits in both directions; simultaneous changes; delete/rename/missing-image recovery; account switching; offline and interrupted file exchange; old-collage recovery after a Mac edit; no active-presentation changes; final wallpaper selection and Photos-save failure handling; actual camera capture and quick actions with unsaved work; and native keyboard, accessibility, light/dark and receiver checks. Public captures use synthetic media. A model review, compile or schema deployment is not a substitute for observed behavior.
