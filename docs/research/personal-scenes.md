...
Product and implementation record, **13 September 2026**. The new scene source is implemented and under validation, not publicly released. Paired scene sync remains **unverified**. The [public visual guide](https://workbench-mac.vercel.app/scenes/) explains these decisions; concepts are separate from running-app evidence.
...
| --- | --- | --- |
| Installed Mac Preview | The new signed app built, installed and opened, retaining the existing photo and seven adopted legacy scenes. The native eight-portrait chooser and card editor were visibly checked. A prepared two-role floating menu switched between only those role labels. Normal Mac opt-in reported In iCloud, with eight matching revision receipts in its durable manifest. An earlier owner-alias correction allowed an existing real iPhone photo to download after a transient timeout and retry. | Paired editable-scene transfer and a reliable transfer-time bound. No private photo is published. |
...
| Shared scene/storage and scheduling | Strict Swift 6 suite: **48 tests, zero failures**, including 26 lifecycle tests; the integrated repository suite also passed all 73 photo and scene tests; shared sources also passed iOS typechecking. Synthetic transport covers coalescing, no-op/receipt loops, background cancellation, durable waiting, manual retry, stale acknowledgements, conflicts and accounts. | Paired scene transfer and recovery. Real Mac opt-in created the private zone and all eight local scenes received matching server revision receipts. The 800 ms value is a local scheduling delay, not measured arrival time. |
| Mobile scene editor | Actual iOS source/test targets compile; nine focused native editor tests passed with synthetic stores. Seven separate bridge checks cover conversion/recovery. | Final installed mobile scene journey, camera/Photos, quick actions, physical devices and receivers. |
| Scene schema/provisioning | Apple’s Production deployment receipt was observed for exactly two additive `SceneV1` fields: `metadata` (BYTES) and `packageAsset` (ASSET). No new public RBAC grants; `PhotoV1` and `Users` were unchanged. The client is responsible for creating its private zone on opt-in. | Normal Mac upload has succeeded for all eight scenes; paired receiver and round-trip behavior remain **unverified**. This implementation is **not publicly released**. No App Store or TestFlight submission has been made. |

Before claiming the scene journey complete, record offline creation/restart; edits in both directions; simultaneous changes; delete/rename/missing-image recovery; account switching; export/import without cloud; old-collage recovery after a Mac edit; no active-presentation changes; actual Photos save; cold/warm quick actions with unsaved work; and native keyboard, accessibility, light/dark and receiver checks. Public captures use synthetic media. A model review, compile or schema deployment is not a substitute for observed behavior.
